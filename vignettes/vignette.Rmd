---
title: "An Introduction to GWASLasso"
...
GWASLasso is a package that is used to fit the lasso on big genomics data. We assume the data are stored in bed/bim/fam format by the [PLINK library](https://www.cog-genomics.org/plink2/formats#bed), and the training part is named by train.bed/bim/fam and if provided the validation part by val.bed/bim/fam.
...
The most important parameters in the core function `GWASLasso` include:
...
```{r}
genotype.dir <- paste0(system.file("extdata", package = "GWASLasso"), "/")
phenotype.file <- system.file("extdata", "sample_phe.phe", package = "GWASLasso")
